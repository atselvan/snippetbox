...
import (
	...
	"github.com/atselvan/snippetbox/internal/validator"
...
type userAccountPasswordUpdateForm struct {
...
func (app *application) userAccountPasswordUpdate(w http.ResponseWriter, r *http.Request) {
	data := app.newTemplateData(r)
    data.Form = userAccountPasswordUpdateForm{}

    app.render(w, http.StatusOK, "password.tmpl", data)
}

func (app *application) userAccountPasswordUpdatePost(w http.ResponseWriter, r *http.Request) {
	var form userAccountPasswordUpdateForm
	...
	form.CheckField(validator.Equal[string](form.NewPassword, form.NewPasswordConfirmation), "newPasswordConfirmation", "Passwords do not match")
	...
	if err != nil {
		if errors.Is(err, models.ErrInvalidCredentials) {
			form.AddFieldError("currentPassword", "Password is invalid")
